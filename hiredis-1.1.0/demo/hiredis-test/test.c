...
static void test_reply_reader(void) {
    ...
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Protocol error, got \"@\" as reply type byte") == 0);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Bad integer value") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Multi-bulk length out of range") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Bulk string length out of range") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR && _stricmp(reader->errstr, "Out of memory") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Bad double value") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Bad nil value") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
    ...
    ret = redisReaderGetReply(reader,&reply);
    test_cond(ret == REDIS_ERR &&
              _stricmp(reader->errstr,"Bad bool value") == 0);
    freeReplyObject(reply);
    redisReaderFree(reader);
...
static void test_blocking_connection(struct config config) {
    ...
    test_cond(reply->type == REDIS_REPLY_STATUS &&
        _stricmp(reply->str,"pong") == 0)
    freeReplyObject(reply);

    test("Is a able to send commands verbatim: ");
    reply = redisCommand(c,"SET foo bar");
    test_cond (reply->type == REDIS_REPLY_STATUS &&
        _stricmp(reply->str,"ok") == 0)
    freeReplyObject(reply);
    ...
    test_cond(reply->type == REDIS_REPLY_ARRAY &&
              ...
              _stricmp(reply->element[1]->str,"pong") == 0);
    freeReplyObject(reply);
...
static void test_blocking_io_errors(struct config config) {
    ...
    if (major > 2 || (major == 2 && minor > 0)) {
        ...
        test_cond(_stricmp(reply->str,"OK") == 0 &&
